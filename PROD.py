...
def calculate_loss(model_disprefered_logits, ground_truth_distribution, loss_mask):
...
def get_output_distribution(logits, labels, loss_mask, top_p=0.8, alpha=0.0, temperature=0.8, N=1, max_N=10):
    probs = F.softmax(logits, dim=-1)
    ...
    with torch.no_grad():
        ...
        one_hot = F.one_hot(labels, num_classes=probs.size(-1)).bool() & shifted_loss_mask.unsqueeze(-1)
...
def collate_fn(batch, tokenizer, max_length):
    ...
    for item in batch:
        ...
        # Tạo mask: 0 cho prompt, 1 cho response
        mask = [0] * len(prompt_tokens) + [1] * (len(full_ids) - len(prompt_tokens))
